...
export default function LeadsPage() {
  ...
  if (loading) return <div className="flex flex-1 items-center justify-center text-sm text-zinc-400">Loading your pipeline…</div>
    ...
    <div className="min-h-full w-full bg-zinc-50/70 p-4 sm:p-6 dark:bg-zinc-950">
      <div className="mx-auto max-w-[1600px]">
        <header className="mb-7 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="mb-2 text-xs font-semibold uppercase tracking-[0.18em] text-emerald-600 dark:text-emerald-400">CRM · Pipeline</p>
            <h1 className="text-3xl font-semibold tracking-tight text-zinc-950 dark:text-white">Keep momentum visible.</h1>
            <p className="mt-2 text-sm text-zinc-500 dark:text-zinc-400">Move conversations forward without losing the context behind them.</p>
          </div>
          <button onClick={() => setShowNew(true)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm shadow-emerald-600/20 transition hover:-translate-y-0.5 hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 dark:focus:ring-offset-zinc-950">
        ...
        <section className="mb-6 grid gap-3 sm:grid-cols-3" aria-label="Pipeline overview">
          ...
          <Metric label="Converted" value={wonCount} icon={<CheckCircle2 className="size-4" />} accent="text-emerald-600 dark:text-emerald-400" />
        </section>

        <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="relative max-w-sm flex-1">
            <Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-zinc-400" />
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search people or sources" className="w-full rounded-xl border border-zinc-200 bg-white py-2.5 pl-9 pr-4 text-sm text-zinc-900 outline-none transition placeholder:text-zinc-400 focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 dark:border-zinc-800 dark:bg-zinc-900 dark:text-white" />
          </div>
          <p className="text-sm text-zinc-500 dark:text-zinc-400"><span className="font-medium text-zinc-900 dark:text-white">{visibleLeads.length}</span> of {leads.length} leads</p>
        </div>
        ...
        {showNew && (
          <form onSubmit={(event) => { event.preventDefault(); createLead() }} className="animate-in mb-6 rounded-2xl border border-emerald-100 bg-white p-4 shadow-sm dark:border-emerald-900/70 dark:bg-zinc-900">
            <div className="mb-4 flex items-center justify-between"><div><h2 className="font-semibold text-zinc-900 dark:text-white">Add a lead</h2><p className="text-sm text-zinc-500">Start with the essentials. You can enrich it later.</p></div><button type="button" onClick={() => setShowNew(false)} className="rounded-lg p-2 text-zinc-400 transition hover:bg-zinc-100 hover:text-zinc-900 dark:hover:bg-zinc-800 dark:hover:text-white" aria-label="Close form"><X className="size-4" /></button></div>
            ...
            <div className="mt-4 flex justify-end gap-2"><button type="button" onClick={() => setShowNew(false)} className="rounded-lg px-3 py-2 text-sm font-medium text-zinc-600 hover:bg-zinc-100 dark:text-zinc-300 dark:hover:bg-zinc-800">Cancel</button><button disabled={isPending} className="rounded-lg bg-emerald-600 px-3 py-2 text-sm font-semibold text-white transition hover:bg-emerald-700 disabled:opacity-60">Create lead</button></div>
          </form>
        )}

        <section className="flex gap-4 overflow-x-auto pb-5" aria-label="Lead pipeline">
          {stages.map((stage) => {
            ...
            return <div key={stage} className="flex w-[280px] shrink-0 flex-col rounded-2xl border border-zinc-200/80 bg-zinc-100/60 p-3 dark:border-zinc-800 dark:bg-zinc-900/40">
              <div className="mb-3 flex items-center justify-between px-1"><div className="flex items-center gap-2"><span className={`size-2 rounded-full ${stageStyle[stage].dot}`} /><h2 className="text-sm font-semibold text-zinc-800 dark:text-zinc-100">{stage}</h2></div><span className="rounded-full bg-white px-2 py-0.5 text-xs font-medium text-zinc-500 shadow-sm dark:bg-zinc-800">{stageLeads.length}</span></div>
              <div className="min-h-28 space-y-2">{stageLeads.map((lead) => <LeadCard key={lead.id} lead={lead} stage={stage} onAdvance={moveStage} />)}{stageLeads.length === 0 && <p className="rounded-xl border border-dashed border-zinc-300 px-3 py-7 text-center text-xs text-zinc-400 dark:border-zinc-700">No leads here yet</p>}</div>
            </div>
          })}
        </section>
      </div>
...
function LeadCard({ lead, stage, onAdvance }: { lead: LeadRecord; stage: (typeof stages)[number]; onAdvance: (id: string, stage: string) => void }) {
  ...
  return <article className="group rounded-xl border border-zinc-200 bg-white p-3.5 shadow-sm transition duration-200 hover:-translate-y-0.5 hover:border-emerald-200 hover:shadow-md dark:border-zinc-800 dark:bg-zinc-900 dark:hover:border-emerald-900"><Link href={`/leads/${lead.id}`} className="block focus:outline-none"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h3 className="truncate text-sm font-semibold text-zinc-900 group-hover:text-emerald-700 dark:text-white dark:group-hover:text-emerald-400">{lead.name}</h3>{lead.email && <p className="mt-1 flex items-center gap-1 truncate text-xs text-zinc-500"><Mail className="size-3 shrink-0" />{lead.email}</p>}</div><span className={`mt-0.5 size-2 shrink-0 rounded-full ${stageStyle[stage].dot}`} /></div>{lead.source && <span className={`mt-3 inline-flex rounded-md px-2 py-1 text-[11px] font-medium ring-1 ring-inset ${stageStyle[stage].badge}`}>{lead.source}</span>}</Link>{!closed && <div className="mt-3 flex items-center justify-between border-t border-zinc-100 pt-2.5 dark:border-zinc-800"><button onClick={() => onAdvance(lead.id, nextStage)} className="inline-flex items-center gap-1 text-xs font-semibold text-emerald-700 transition hover:text-emerald-900 dark:text-emerald-400 dark:hover:text-emerald-300">Advance <ArrowRight className="size-3" /></button><button onClick={() => onAdvance(lead.id, 'Lost')} className="text-xs font-medium text-zinc-400 transition hover:text-rose-600">Mark lost</button></div>}</article>
}
